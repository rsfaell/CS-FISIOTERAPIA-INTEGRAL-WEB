...
export default function FinanceiroPaciente({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function carregarDados() {
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Ajuste de tipagem para acessar o nome do paciente corretamente
        const nomeData = data[0].pacientes;
        const nome = Array.isArray(nomeData) ? nomeData[0]?.nome : (nomeData as any)?.nome;
  ...
  return (
    <div className="relative min-h-screen w-full bg-white text-slate-800 font-sans overflow-hidden p-6 md:p-12">
      ...
      <div className="absolute inset-0 flex items-center justify-center opacity-[0.04] pointer-events-none grayscale">
        <img src="/logocs.png" alt="" className="w-full max-w-3xl object-contain" />
      </div>
      ...
      {/* Modal de Edição com Acessibilidade Corrigida */}
      {itemParaEditar && (
            ...
            <div className="space-y-4">
              <div>
                ...
                <select 
                  ...
                  className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-slate-700 outline-none uppercase text-xs focus:ring-2 focus:ring-teal-500/20" 
                ...
                <>
                  <div>
                    ...
                    <input 
                      id="modal-valor"
                      type="text" 
                      className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-slate-700 outline-none focus:ring-2 focus:ring-teal-500/20" 
