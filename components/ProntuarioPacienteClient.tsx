...
export default function ProntuarioPacienteClient({ id }: { id: string }) {
  ...
  const [novoValor, setNovaValor] = useState<number>(100);
  ...
  async function excluirPaciente() {
    if (!confirm("Tem certeza que deseja excluir este paciente permanentemente? Todos os agendamentos e evoluções também serão perdidos.")) return;
    
    // Apaga os agendamentos primeiro para evitar erros de chave estrangeira
    await supabase.from('agenda').delete().eq('paciente_id', id);
    
    // Em seguida apaga o paciente
    const { error } = await supabase.from('pacientes').delete().eq('id', id);
    
    if (!error) {
      router.push('/pacientes');
    } else {
      alert("Ocorreu um erro ao tentar excluir o paciente. Tente novamente.");
    }
  }

  async function marcarSessao(e: React.FormEvent) {
    e.preventDefault();
    const valorFinal = novaFormaPagamento === 'gratis' ? 0 : novoValor;
  ...
  async function salvarAjuste(sessaoId: string) {
    const valorAjustado = editPagamento === 'gratis' ? 0 : editValor;
      ...
      <div className="relative z-10 flex flex-col h-full w-full">
        <header className="w-full p-6 flex justify-between items-center border-b border-slate-50 bg-white/80 backdrop-blur-md">
          <div className="flex items-center gap-4">
            <Link href="/prontuario" className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors">
            ...
            <div className="bg-white/60 backdrop-blur-md p-6 rounded-[35px] border border-slate-100 shadow-sm">
              ...
              {isEditandoPaciente ? (
                <form onSubmit={salvarDadosPaciente} className="space-y-3 animate-in fade-in duration-300">
                  <input type="text" placeholder="Nome" aria-label="Nome do Paciente" className="w-full p-2 bg-slate-50 rounded-lg text-[10px] font-bold outline-none" value={editNome} onChange={e => setEditNome(e.target.value)} />
                  <input type="text" placeholder="CPF" aria-label="CPF" className="w-full p-2 bg-slate-50 rounded-lg text-[10px] font-bold outline-none" value={editCpf} onChange={e => setEditCpf(e.target.value)} />
                  <input type="text" placeholder="Telefone" aria-label="Telefone" className="w-full p-2 bg-slate-50 rounded-lg text-[10px] font-bold outline-none" value={editTelefone} onChange={e => setEditTelefone(e.target.value)} />
                  <div className="flex gap-2 pt-1">
                    <button type="submit" className="flex-1 py-2 bg-teal-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest transition-colors hover:bg-teal-700">Salvar</button>
                    <button type="button" onClick={excluirPaciente} className="px-4 py-2 bg-red-50 text-red-500 rounded-lg text-[9px] font-black uppercase tracking-widest transition-colors hover:bg-red-100" title="Excluir Paciente">Excluir</button>
                  </div>
                </form>
              ) : (
              ...
              {!isAgendando ? (
                ...
                <form onSubmit={marcarSessao} className="space-y-4 animate-in fade-in zoom-in duration-300">
                  ...
                  {novaFormaPagamento !== 'gratis' && (
                    <input type="number" placeholder="Valor R$" className="w-full p-3 rounded-xl bg-slate-50 border-none text-[10px] font-bold" value={novoValor} onChange={e => setNovaValor(Number(e.target.value))} />
                  )}
                ...
                {editandoSessaoId === sessao.id ? (
                  <div className="space-y-4 animate-in slide-in-from-top-2 duration-200">
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      {editPagamento !== 'gratis' && <input type="number" placeholder="Valor R$" className="p-2 bg-slate-50 rounded-lg text-[10px] font-bold" value={editValor} onChange={e => setEditValor(Number(e.target.value))} />}
                    </div>
                    <textarea
                  ...
                  <>
                    ...
                    <div className="p-4 bg-white/40 rounded-2xl border border-slate-50 text-xs text-slate-500 whitespace-pre-wrap">{sessao.evolucao || "Sem anotações de evolução."}</div>
                  </>
                )}
              </article>
            ))}
